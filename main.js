...
const randomPhone = () => {
    let numbers = []
    arr.map(item => {
        let randomNumber = item.phone.split('').map(() => Math.floor(Math.random() *10)).join('')
        item.phone = randomNumber
        numbers.push(randomNumber)
        maximum.push(Math.max(randomNumber))
    })
    console.log(numbers);
}
...
const randomAddress = () => {
    const address = arr[Math.floor(Math.random() * arr.length)].address;
    return address;
}
...
const randomEmail = () => {
    const email = arr[Math.floor(Math.random() * arr.length)].email;
    return email;
}
